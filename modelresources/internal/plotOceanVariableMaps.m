function plotOceanVariableMaps(oceanVar,lonVector,latVector,myColourMap,cbString,...
    caxisMin,caxisMax,isCommonColourBar,labelVars,figureName,folderName,sgString)
...
%       folderName        - string specifying the subfolder in ./figures
...
%       saveFigureInFolder - custom function with type of figure output
...
elseif (nSubplots == 2 && ~isCommonColourBar)
    
    figure()
    set(gcf,'Units','Normalized','Position',[0.01 0.05 0.40 0.25],'Color','w')
    haxis = zeros(nSubplots,1);

    for iSubplot = 1:nSubplots

        haxis(iSubplot) = subaxis(1,2,iSubplot,'Spacing',0.01,'Padding',0.02,'Margin',0.04);
        ax(iSubplot).pos = get(haxis(iSubplot),'Position');
...
elseif (nSubplots == 14 && ~isCommonColourBar) % formerly (nSubplots == 15 && ~isCommonColourBar)
    ...
    for iSubplot = 1:nSubplots
        ...
        haxis(iSubplot) = subaxis(5,3,iSubplot,'Spacing',0.025,'Padding',0.015,'Margin',0.010);
        ax(iSubplot).pos = get(haxis(iSubplot),'Position');
        % if iSubplot < 4
        %     ax(iSubplot).pos(2) = ax(iSubplot).pos(2) + 0.02; 
        % elseif (iSubplot >=4 && iSubplot < 7)
        %     ax(iSubplot).pos(2) = ax(iSubplot).pos(2) + 0.01; 
        % elseif (iSubplot >=7 && iSubplot < 10)
        %     ax(iSubplot).pos(2) = ax(iSubplot).pos(2);
        % elseif (iSubplot >=10 && iSubplot < 13)
        %     ax(iSubplot).pos(2) = ax(iSubplot).pos(2) - 0.01; 
        % elseif (iSubplot >=13)
        %     ax(iSubplot).pos(2) = ax(iSubplot).pos(2) - 0.02; 
        % end
        ...
        mydata = oceanVar(:,:,iSubplot);
        plotPcolorMap(haxis(iSubplot),lonVector,latVector,mydata,myColourMap{iSubplot},...
            caxisMin(iSubplot),caxisMax(iSubplot));
        title(labelVars(iSubplot),'FontSize',12) 

        cb = colorbar('Location','southoutside');
        cb.Position(1) = cb.Position(1) + 0.026;
        cb.Position(2) = cb.Position(2) - 0.025;
        cb.Position(3) = 0.15; % length
        cb.Position(4) = 0.005; % width

        if ~isempty(cbString(iSubplot))
            if contains(cbString(iSubplot),'log10')
                nTicks = (caxisMax(iSubplot) - caxisMin(iSubplot))+1;
                ticks = linspace(caxisMin(iSubplot),caxisMax(iSubplot),nTicks);
                % Adjusts the format dynamically, ensuring the number of decimals 
                % is appropriate based on the scale
        ...
        cb.Label.Rotation = 0;
        cb.FontSize = 8;
        cb.Label.Position(2) = cb.Label.Position(2) + 0.2;
...
saveFigureInFolder(folderName,figureName)
